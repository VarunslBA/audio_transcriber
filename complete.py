...
if __name__ == "__main__":
    ...
    # Print timestamp for the first entry
    # print(f"Timestamp: {combined_results[0]['start_time']}s - {combined_results[0]['end_time']}s")
    # print(f"Speaker: {combined_results[0]['speaker_id']}")
    # print(f"Detected Language: {combined_results[0]['detected_language']}")
    # print(f"Transcript: {combined_results[0]['transcript']}")
    print("\n")
    print(f"{combined_results[0]['speaker_id']}: {combined_results[0]['transcript']}")

    current_speaker = combined_results[0]['speaker_id']
    for entry in combined_results[1:]:
        if current_speaker != entry['speaker_id']:
            print("\n")
            # print(f"Timestamp: {entry['start_time']}s - {entry['end_time']}s")
            # print(f"Speaker: {entry['speaker_id']}")
            # print(f"Detected Language: {entry['detected_language']}")
            # print(f"Transcript: {entry['transcript']}")
            # current_speaker = entry['speaker_id']
            print(f"{entry['speaker_id']}: {entry['transcript']}")
        else:
